...
public class date_time_tests
{
    ...
    [Fact]
    public void defined_date_time_with_min_date_time_constraint()
    {
        var dateTimeField = new DateTimeField(new DynamicFieldId<Guid>(Guid.NewGuid()), "DateTime",
            "DateTime", false, new List<Constraint<System.DateTime>>
            {
                new MinDateTimeConstraint(new System.DateTime(2024, 01, 01))
            });
        var dateTimeException = () => dateTimeField.SetValue(new System.DateTime(2023, 10, 10));
        Assert.Throws<MinDateTimeException>(dateTimeException);
    }
}